...
QBOConnector.prototype.generateSetupUrl = function (bounce) {
  ...
  return BBPromise.try(function () {
    ...
    return 'https://appcenter.intuit.com/Connect/SessionStart?grantUrl=' + encodeURIComponent('https://' + config.get('Hoist.domains.bouncer') + '/bounce') + '&datasources=' + encodeURIComponent(datasources.join());
  });
};
...
QBOConnector.prototype.getRootUrl = function () {
  ...
  if (this.settings.isSandbox) {
    domain = 'sandbox--quickbooks-api-intuit-com-s404txkcwglf.runscope.net';
    var originalDomain = 'sandbox-quickbooks.api.intuit.com';
    ...
    this.auth._createSignatureBase = function (method, u, parameters) {
      return this._createSignatureBaseOriginal(method, u.replace(domain, originalDomain), parameters);
    };
  }
